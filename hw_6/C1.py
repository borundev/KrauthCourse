...
for step in range(n_steps):
    ...
    Trotter_weight_old = math.exp(sum(-V(a, g_cubic, g_quartic) * dtau for a in x))
    Trotter_weight_new = math.exp(sum(-V(a, g_cubic, g_quartic) * dtau for a in x_new))
...
pylab.title('Path Sample for C1')
pylab.savefig('x_positions_C1.png')
pylab.clf()
...
pylab.title('anharmonic_path C1 (beta=%s, N=%i)' % (beta, N))
#pylab.xlim(-2, 2)
pylab.savefig('plot_C1_beta%s_levy.png' % beta)
pylab.show()
